    ...
    if (spotify.playing && spotify.track && spotify.track !== prevTrack.current) {
      if (prevTrack.current) {
        setRecentTracks(prev => [
          { n: prevTrack.current, a: spotify.artist, ts: new Date().toLocaleTimeString('en-GB', { hour:'2-digit', minute:'2-digit' }) },
          ...prev
        ].slice(0, 8));
      }
    ...
    >
      {isEco ? (
        <div className="flex flex-col items-center justify-center py-10 gap-3">
          ...
          <div className="text-[13px] font-semibold text-[#1a1a1a] dark:text-[#e8e8ea]">{t.ecoNoSpotify}</div>
          <div className="text-[11px] text-black/30 dark:text-white/30 text-center max-w-[220px]">
            Switch to Normal or Turbo mode in Settings to enable music tracking.
          </div>
        </div>
      ) : (
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="bg-black/[0.03] dark:bg-white/[0.04] rounded-none-none p-3 border border-black/[0.05] dark:border-white/[0.06]">
              ...
              {spotify.playing ? (
                ...
                <div className="text-[10px] text-black/20 dark:text-white/20 mt-1">{t.notPlaying}</div>
              )}
            </div>
            ...
            <div className="bg-black/[0.03] dark:bg-white/[0.04] rounded-none-none p-3 border border-black/[0.05] dark:border-white/[0.06] flex-1 flex flex-col">
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-2 py-3">
                ...
                <div className="text-[11px] font-semibold text-[#1a1a1a] dark:text-[#e8e8ea] text-center">Premium Feature</div>
                <div className="text-[10px] text-black/30 dark:text-white/30 text-center">Live lyrics require a<br/>Spotify Premium token.</div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5 overflow-y-auto max-h-[420px]">
              ...
              {recentTracks.map((tr, i) => (
                <div key={i} className="flex items-center gap-2 px-2.5 py-1.5 rounded-none-none bg-black/[0.02] dark:bg-white/[0.03] border border-black/[0.04] dark:border-white/[0.05] hover:bg-black/[0.04] dark:hover:bg-white/[0.06] transition-colors">
                  ...
                  <span className="text-[9px] text-black/20 dark:text-white/20">{tr.ts}</span>
                </div>
              ))}
              ...
              {!spotify.playing && recentTracks.length === 0 && (
                <div className="text-center py-10 text-black/20 dark:text-white/20">
                  ...
                  <div className="text-[11px]">{t.openSpotify}<br/>to see track info here</div>
                </div>
              )}
            </div>
          </div>
